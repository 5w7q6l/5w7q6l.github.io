...
def train_model():
    ...
    with open("data/glove.{}d.vector".format(embedding_size), "w", encoding="utf-8") as f:
        for i, word in enumerate(word2idx.keys()):
            if i == 0:
                continue
            vec = model.center_embed.weight.data[i] + model.backgroud_embed.weight.data[i]
            vec_str = ' '.join([str(x) for x in vec.cpu().numpy()])
            f.write("{} {}\n".format(word, vec_str))
    torch.save(model.state_dict(), "data/glove_model.model")
